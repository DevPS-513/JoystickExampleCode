...
#include "Joystick.h"
 ...
 Joystick::Joystick(int pwm1,int dir1,int pwm2, int dir2,int x_axis_min, 
					...
					float theta_range,int center_point,float sensor_wavenumber, int x_sensor_pin,int y_sensor_pin)                     
{ 	

// Assign private variables as needed
	_pwm1_pin=pwm1;
	_pwm2_pin=pwm2;
  ...
  h_center= (float) center_point;
	...
	_h_wavenumber=sensor_wavenumber; // relates sine wave to actual degrees
...
{
   x_pos_unity= (float) (analogRead(x_pin)-x_min);
...
{
   y_pos_unity= (float) (analogRead(y_pin)-y_min);
